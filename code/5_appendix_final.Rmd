...
author: |
  Dorit Talia Stein, Marissa B. Reitsma, Pascal Geldsetzer, Kokou Agoudavi, Krishna Kumar Aryal, Silver Bahendeka,     
  Luisa C C Brant, Farshad Farzadfar, Mongal Singh Gurung, David Guwatudde, Yessito Corine Nadège Houehanou,   
  Deborah Carvalho Malta, João Soares Martins, Sahar Saeedi Moghaddam, Kibachio Joseph Mwangi, Bolormaa Norov,   
  Lela Sturua, Zhaxybay Zhumadilov, Till Bärnighausen, Justine I. Davies, David Flood,   
  Maja E. Marcus, Michaela Theilmann, Sebastian Vollmer, Jennifer Manne-Goehler, Rifat Atun, Nikkil Sudharsanan,   
  Stéphane Verguet
output:
  pdf_document:
    toc: yes
...
### Supplementary Table `r count<- count+1; count`. Mean (95% confidence interval) proportion of people living with hypertension aware of their diagnosis, on treatment, and with controlled hypertension by country and wealth quintile
...
cascade_table <- cascades %>% 
  ...
  mutate(across(c(value, lower, upper), ~sprintf("%.1f", round(., 1)))) %>%
                #  round(.,1))) %>%
  ungroup() %>%
  mutate(bounds = ifelse(is.na(lower), 
                         value, 
                         paste0(value, " ", "(", lower, ",", upper, ")"))) %>%
...
### Supplementary Table `r count<- count+1; count`. Mean (95% confidence interval) proportion of people living with hypertension aware of their diagnosis, on treatment, and with controlled hypertension by wealth quintile and country-level income group
...
### Supplementary Table `r count<- count+1; count`. Ranked gap in hypertension care cascade performance comparing top (Q5) and bottom (Q1) wealth quintiles at baseline by country ('baseline cascade gap')
...
kbl(auc_table,
  ...
  footnote(general_title = "Note.", 
           footnote_as_chunk = TRUE,
           threeparttable = TRUE,
           general = "Performance gap was calculated by taking the difference in the area under the cascade between the top and bottom wealth quintiles in each country. Countries were sorted by the performance gap and split in terciles representing the top, middle, and bottom third across countries. Tercile 1 corresponds to the lowest ranking countries while Tercile 3 is the highest ranking countries (i.e. countries with the largest difference in area under the cascades between the bottom and top wealth quintiles).")
...
diagnosis <- all %>%
  ...
  mutate(across(c(linkage, aware_target2, resulting_tx), 
                ~sprintf("%.1f", round(., 1)))) %>%
                #~round(.,1))) %>%
  arrange(Country, wealth_quintile) %>%
...
#write.csv(diagnosis, "3 Output/suppl-5-diagnosis-link-tx.csv")
...
targets_perf <- all %>%
  ...
  mutate(aware_target_perf = sprintf("%.1f", round(aware_target_perf*100, 1)),
         #~sprintf("%.1f", round(., 1)))) %>%
         tx_target_perf = sprintf("%.1f", round(tx_target_perf*100, 1))) %>%
  distinct() %>%
...
targets_rel <- all %>%
  ...
  distinct() %>%
  arrange(Country, wealth_quintile) %>%
  mutate(aware_target_rel = sprintf("%.1f", round(aware_target_rel*100, 1)),
         tx_target_rel = sprintf("%.1f", round(tx_target_rel*100, 1))) %>%
...
targets_sens <- all %>%
  ...
  distinct() %>%
  arrange(Country) %>%
  mutate(aware_target_sens = sprintf("%.1f", round(aware_target_sens*100, 1)),
         tx_target_sens = sprintf("%.1f", round(tx_target_sens*100, 1))) %>%
...
cvd_table <- mean_risk %>% 
  ...
  mutate(across(c(mean, lower, upper), ~sprintf("%.1f", round(.,1)))) %>%
...
kbl(cvd_table2,
  ...
  footnote(general_title = "Note.", 
           footnote_as_chunk = TRUE,
           threeparttable = TRUE,
           general = "95% confidence intervals are presented for the Baseline scenario. 95% uncertainty intervals are presented for the Diagnosis and Treatment scenario values.")
```

\newpage

### Supplementary Table `r count<- count+1; count`. Absolute (percentage point) difference in mean 10-year cardiovascular disease risk comparing the bottom (Q1) and top (Q5) wealth quintiles by country and modelled scenario, either Baseline, Diagnosis or Treatment scenario
...
diff_table <- difference_overall %>%
  ...
  mutate(difference1_5 = sprintf("%.1f", round(difference1_5, 2))) %>%
  select(Country = country, Scenario = scenario, 
         "Absolute (percentage point) difference in mean CVD risk (Q1-Q5)" = difference1_5) %>%
...
### Supplementary Table `r count<- count+1; count`. Mean (95% uncertainty interval) relative decline (%) in CVD risk compared to baseline by country, wealth quintile, and scenario
...
rel_table <- rel %>% 
  ...
  mutate(across(c(mean, lower, upper), ~sprintf("%.1f", round(.,1)))) %>%
  ungroup() %>%
  mutate(bounds = paste0(mean, " ", "(", lower, ",", upper, ")")) %>% # with rel decline, upper bound (of mean CVD) is actually lowest value
  dplyr::select(Country = country, 
         Scenario = scenario, 
         "Wealth Quintile" = wealth_quintile, 
...
gap_table1 <- by_inequity_rate %>%
  mutate(averted_rate = sprintf("%.1f", round(averted_rate,1))) %>%
  filter(scenario_type == "overall") %>%
...
gap_table2 <- by_inequity_prop %>%
  ungroup() %>%
  filter(scenario_type == "overall") %>%
  mutate(prop_cases = sprintf("%.1f", round(prop_cases*100,1))) %>%
...
#### Supplementary Table `r count<- count+1; count`. Performance-based targets sensitivity analysis: Mean (95% uncertainty interval) 10-year CVD risk by country and wealth quintile in Diagnosis and Treatment scenarios
...
#### Supplementary Table `r count<- count+1; count`. Performance-based targets sensitivity analysis: Absolute (percentage point) difference in mean 10-year cardiovascular disease risk comparing the bottom (Q1) and top (Q5) wealth quintiles by country and modelled scenario, either Baseline, Diagnosis or Treatment scenario
...
#### Supplementary Table `r count<- count+1; count`. Performance-based targets sensitivity analysis: Mean (95% uncertainty interval) relative decline (%) in CVD risk compared to baseline by country, wealth quintile, and scenario
...
#### Supplementary Table `r count<- count+1; count`. Relative increase targets sensitivity analysis: Mean (95% uncertainty interval) 10-year CVD risk by country and wealth quintile in Diagnosis and Treatment scenarios
...
#### Supplementary Table `r count<- count+1; count`. Relative increase targets sensitivity analysis: Mean (95% uncertainty interval) relative decline (%) in CVD risk compared to baseline by country, wealth quintile, and scenario
...
#### Supplementary Table `r count<- count+1; count`. Within-country targets sensitivity analysis: Mean (95% uncertainty interval) 10-year CVD risk by country and wealth quintile in Diagnosis and Treatment scenarios
...
#### Supplementary Table `r count<- count+1; count`. Within-country targets sensitivity analysis: Absolute (percentage point) difference in mean 10-year cardiovascular disease risk comparing the bottom (Q1) and top (Q5) wealth quintiles by country and modelled scenario, either Baseline, Diagnosis or Treatment scenario
...
#### Supplementary Table `r count<- count+1; count`. Within-country targets sensitivity analysis: Mean difference (95% uncertainty interval) in CVD cases per 1,000 people living with hypertension compared to baseline by country, wealth quintile, and scenario
...
#### Supplementary Table `r count<- count+1; count`. Within country targets sensitivity analysis: Mean (95% uncertainty interval) relative decline (%) in CVD risk compared to baseline by country, wealth quintile, and scenario
...
miss_table <- missing %>%
  mutate(across(.cols = c(age_na_pct, sex_na_pct, sbp_na_pct, smk_na_pct, bmi_na_pct, wealth_na_pct, age_bound_pct), .fns = ~sprintf("%.1f", round(.*100,1)))) %>%
  ...
  mutate('Age, missing, n (%)' = paste0(age_na," (", age_na_pct, "%)"), 
        ...
        ##'N (non-missing age, sex)' = n_agesex,
         'SBP, missing, n (%)' = paste0(sbp_na," (", sbp_na_pct, "%)"), 
         #'SBP, missing (%)' = sbp_na_pct, 
        ##'N (non-missing age, sex, htn status)' = n_agesex_hypt,
...
footnote(general_title = "Note.", 
           footnote_as_chunk = TRUE,
           threeparttable = TRUE,
           general = "Age and sex missingness and out of bounds values are reported on the full survey sample. Systolic Blood Pressure (SBP) missingness is reported on the sample that meet the inclusion criteria for age and sex. Currently smoking, Body Mass Index (BMI), and wealth quintile missingness is reported among the population of people living with hypertension that meet inclusion criteria for age, sex, and have non-missing SBP values. These values do not distinguish between 'do not know/refuse to respond' and 'missing because not eligible/asked'. In some surveys, specific sets of questions were only asked to a subset of the full survey sample.")
...
dots <- ggplot(data = graph, 
  ...
  labs(y = "Mean Decline (%) in CVD Risk from Baseline", x = "",
       caption = "Note: Vertical bars represent 95% uncertainty intervals \n for the Diagnosis and Treatment scenarios.") +
