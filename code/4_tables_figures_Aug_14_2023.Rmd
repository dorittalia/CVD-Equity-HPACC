...
who <- read.csv("1 Raw Data/who-regions.csv") %>%
  rename(ccodes = Code)
...
saveRDS(who_codes, "3 Output/who-region-codes-2020.rds")
...
```{r}
# check how many of everything I have

check_cvd <- mean_risk %>%
  group_by(country, wealth_quintile, scenario_type) %>%
  summarize(n = n())
check_cvd2 <- mean_risk %>%
  group_by(country) %>%
  summarize(n = n())

check_rel <- scenario_calcs_rel %>%
  group_by(country, wealth_quintile, scenario_type) %>%
  summarize(n = n())

check_abs <- scenario_calcs2 %>%
  group_by(country, scenario_type, name) %>%
  summarize(n=n())

```

# check N# per WHO region, countrygdp class
```{r}

region_n <- scenario_calcs2 %>%
  distinct(country, WHO_region) %>%
  group_by(WHO_region) %>%
  summarize(n = n())

lmic_n <- scenario_calcs2 %>%
  distinct(country, countryGDPclass) %>%
  group_by(countryGDPclass) %>%
  summarize(n = n())
...
ggplot(data = df, 
  ...
  ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure1-cascades-vars-Jun13.eps", width = 10, height = 5, dpi = 600)


```

# Figure 1 v2 (BY WHO REGION)
Calculate mean + 95% CI for country groups (low, low-middle, middle-income)
...
data_n2 <- data6 %>%
  group_by(WHO_region, wealth_quintile) %>%
  summarize(prop_aware = survey_mean(bpms_hypt, proportion = T, vartype = "ci"),
            prop_tx = survey_mean(bpms_hypt_med, proportion = T, vartype = "ci"),
            prop_control = survey_mean(bpms_hypt_med_ctl, proportion = T, vartype = "ci")) %>%
  mutate_at(vars(prop_aware:prop_control_upp), ~(round((. * 100),2)))
...
#get data into long format for ggplot
cascade_q_data2 <- data_n2 %>%
  select(WHO_region, wealth_quintile, 
         prop_aware, 
         prop_tx,  
        prop_control) %>%
  pivot_longer(prop_aware:prop_control, names_to = "cascade_variable") 
...
lower2 <- data_n2 %>%
  ...
  mutate(cascade_variable = case_when(cascade_variable == "prop_aware_low"  ~ "prop_aware",
                                      ...
                                      cascade_variable == "prop_control_low" ~ "prop_control",
                                      TRUE ~ cascade_variable)) %>%
  select(WHO_region, wealth_quintile,cascade_variable,
         lower = value)
...
#join so have column with cascade variable names in each row and the upper and lower bound values next to it for ggplot format
cascade_q_data3 <- cascade_q_data2 %>% left_join(lower2) %>% left_join(upper2)

#factor cascade levels
cascade_q_data3$cascade_variable <- factor(cascade_q_data3$cascade_variable, 
                                          levels = c("prop_aware", "prop_tx", "prop_control"))
cascade_q_data3$wealth_quintile <- as.factor(cascade_q_data3$wealth_quintile)
...
prop_htn3 <- cascade_q_data3 %>%
  group_by(wealth_quintile, WHO_region) %>%
 summarize(cascade_variable = "prop_htn",
         value = 100,
         lower = 100,
         upper = 100)

cascade_q_data4 <- cascade_q_data3 %>%
  rbind(prop_htn3) %>%
  arrange(cascade_variable, value)

cascade_q_data4$cascade_variable <- factor(cascade_q_data4$cascade_variable,
                                        levels = c("prop_htn","prop_aware","prop_tx", "prop_control"),
                                        labels = c("Hypertensives", "Aware", "Treated", "Controlled"))
cascade_q_data4$wealth_quintile <- factor(cascade_q_data4$wealth_quintile,
                                     levels = c("1", "2", "3", "4", "5"),
                                      labels= c("Q1", "Q2", "Q3", "Q4", "Q5"))

saveRDS(cascade_q_data4, "3 Output/cascade-graph-region-group.rds")
...
cascade_country2 <- readRDS("3 Output/cascade-graph-region-group.rds")
...
ggplot(data = df2, 
  ...
  ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure1-cascades-regions.eps", width = 8, height = 8, dpi = 600)
...
plot_cvd_difference <- function(graph_cvd2, title) {
  ...
  ggplot(data = graph_cvd2, aes(x = difference1_5, 
    ...
    geom_point(size = 2.5) + 
    geom_hline(yintercept = seq(1, length(graph_cvd2$country), by = 1), color = "gray", size = 0.3, alpha = 0.1) + 
    theme_classic() +
...
# Create a separate legend plot for the country colors
legend <- ggplot(data = difference_overall, aes(x = difference1_5, 
                                y = countryGDPclass,
                                color = as.factor(countryGDPclass))) +
    geom_point(size=2.5) +
    scale_color_manual(values = unique(difference_overall$country_colors)) +
  labs(color = "Country-level Income Group") +
  theme_classic() +
  theme(legend.text = element_text(size = 12),
          strip.text = element_text(size = 13),
          legend.title = element_text(size = 14))

# cowplot pulls out legend
legend2 <- get_legend(legend)


# Plot by scenario_type
overall_f2 <- plot_cvd_difference(difference_overall, title = "")
#overall_f2_2 <- plot_grid(overall_f2, legend2, ncol = 1, rel_widths = c(0.8, 0.2))

legend1 <- get_legend(overall_f2)

ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure2-difference-overall.png", width = 12, height = 8)

#income_f2 <- plot_cvd_difference(difference_income, title = "Income-based Scenario Targets")
#ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure2-difference-income.png", #width = 12, height = 8)

perf_f2 <- plot_cvd_difference(difference_perf, title = "Performance-based Targets")
...
sens_f2 <- plot_cvd_difference(difference_sens, title = "Within-country Targets")
...
combined_plot
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure2-all.png", width = 12, height = 20)
...
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure3_gdp_overall.png", fig3_gdp_overall, width = 12, height = 8)

fig3_perf_overall <- fig3(by_perf_rate, "overall", "Overall Targets", area_sum_rank, 45)

# EDF- income  based
fig3_gdp_income <- fig3(by_gdp_rate, "income", "Income-based Targets", countryGDPclass, 35)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/income-based-rate.png", fig3_gdp_income, width = 12, height = 8)

fig3_perf_income <- fig3(by_perf_rate, "income", "Income-based Targets", area_sum_rank, 45)
fig3_gdp_perf <- fig3(by_gdp_rate, "perf", "Performance-based Targets", countryGDPclass, 35)

# EDF - perf based
fig3_perf_perf <- fig3(by_perf_rate, "perf", "Performance-based Targets", area_sum_rank, 45)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/perf-based-rate.png", fig3_perf_perf, width = 12, height = 8)

# EDF - relative
fig3_gdp_rel <- fig3(by_gdp_rate, "rel", "Relative Increase Targets", countryGDPclass, 35)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/relative-rate.png", fig3_gdp_rel, width = 12, height = 8)

# EDF - sens
fig3_gdp_sens <- fig3(by_gdp_rate, "sens", "Within-country Targets", countryGDPclass, 35)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/sens-rate.png", fig3_gdp_sens, width = 12, height = 8)

fig3_perf_rel <- fig3(by_perf_rate, "rel", "Relative Increase Targets", area_sum_rank, 45)

fig3_gdp <- plot_grid(fig3_gdp_overall, fig3_gdp_income, fig3_gdp_perf, fig3_gdp_rel, ncol = 1)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure3_gdp.png", fig3_gdp, width = 12, height = 20)

fig3_perf <- plot_grid(fig3_perf_overall, fig3_perf_income, fig3_perf_perf, fig3_perf_rel, ncol = 1)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure3_perf.png", fig3_perf, width = 12, height = 20)

# WHO regions; scenario type ("overall" only so have same targets across regions)
fig3_region_overall <- fig3(by_region_rate, "overall", "Overall Targets", WHO_region, 35)

#fig3_region_income <- fig3(by_gdp_rate, "income", "Income-based Targets", countryGDPclass, 35)
fig3_region_perf <- fig3(by_region_rate, "perf", "Performance-based Targets", WHO_region, 35)

fig3_region_rel <- fig3(by_region_rate, "rel", "Relative Increase Targets", WHO_region, 35)

ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure3_regions-overall.png", fig3_region_overall, width = 12, height = 8)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure3_regions-perf.png", fig3_region_perf, width = 12, height = 8)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure3_regions-rel.png", fig3_region_rel, width = 12, height = 8)
```
...
fig4 <- function(dataframe, scenario_type_name, title, countryGDPclass, y) {
  
  ggplot(data = dataframe %>% filter(scenario_type == scenario_type_name), 
       aes(x = wealth_quintile, y = prop_cases*100, fill = wealth_quintile)) +
...
fig4_gdp_overall <- fig4(by_gdp_prop, "overall", "", countryGDPclass, 42)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure4_gdp_overall.png", fig4_gdp_overall, width = 12, height = 8)
fig4_perf_overall <- fig4(by_perf_prop, "overall", "Overall Targets", area_sum_rank, 42)

# EDF- income
fig4_gdp_income <- fig4(by_gdp_prop, "income", "Income-based Targets", countryGDPclass, 42)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/income-based-prop.png", fig4_gdp_income, width = 12, height = 8)

fig4_perf_income <- fig4(by_perf_prop, "income", "Income-based Targets", area_sum_rank, 42)
fig4_gdp_perf <- fig4(by_gdp_prop, "perf", "Performance-based Targets", countryGDPclass, 42)

# EDF- performance
fig4_perf_perf <- fig4(by_perf_prop, "perf", "Performance-based Targets", area_sum_rank, 42)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/perf-based-prop.png", fig4_perf_perf, width = 12, height = 8)

# EDF - relative
fig4_gdp_rel <- fig4(by_gdp_prop, "rel", "Relative Increase Targets", countryGDPclass, 42)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/relative-prop.png", fig4_gdp_rel, width = 12, height = 8)

# EDF - sens
fig4_gdp_sens <- fig4(by_gdp_prop, "sens", "Within-country Targets", countryGDPclass, 45)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/sens-prop.png", fig4_gdp_sens, width = 12, height = 8)

fig4_perf_rel <- fig4(by_perf_prop, "rel", "Relative Increase Targets", area_sum_rank, 42)

fig4_gdp <- plot_grid(fig4_gdp_overall, fig4_gdp_income, fig4_gdp_perf, fig4_gdp_rel,ncol = 1)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure4_gdp.png", fig4_gdp, width = 12, height = 20)

fig4_perf <- plot_grid(fig4_perf_overall, fig4_perf_income, fig4_perf_perf, fig4_perf_rel, ncol = 1)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure4_perf.png", fig4_perf, width = 12, height = 20)

# WHO region
fig4_region_overall <- fig4(by_region_prop, "overall", "Overall Targets", WHO_region, 35)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure4_regions-overall.png", fig4_region_overall, width = 12, height = 8)

fig4_region_perf <- fig4(by_region_prop, "perf", "Performance-based Targets", WHO_region, 35)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure4_regions-perf.png", fig4_region_perf, width = 12, height = 8)

fig4_region_rel <- fig4(by_region_prop, "rel", "Relative Increase Targets", WHO_region, 35)
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/figure4_regions-rel.png", fig4_region_rel, width = 12, height = 8)
...
# income based scenario plots
#income_based <- plot_grid(fig3_gdp_income, fig4_gdp_income, ncol = 2, nrow = 1, labels = c('A', 'B'))
#ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/income-based-EDF.png", #income_based, width = 18, height = 12)

# performance based scenario plots
perf_based <- plot_grid(fig3_perf_perf, fig4_perf_perf, ncol = 2, nrow = 1, labels = c('A', 'B'))
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/perf-based-EDF.png", perf_based, width = 18, height = 12)

# relative increase scenario plots
relative <- plot_grid(fig3_gdp_rel, fig4_gdp_rel, ncol = 2, nrow = 1, labels = c('A', 'B'))
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/relative-EDF.png", relative, width = 18, height = 12)

# within country scenario plots
sens <- plot_grid(fig3_gdp_sens, fig4_gdp_sens, ncol = 2, nrow = 1, labels = c('A', 'B'))
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/sens-EDF.png", sens, width = 18, height = 12)
...
grid <- plot_grid(perf_f2 + theme(legend.position = "none"), 
                  rel_f2 + theme(legend.position = "none"),
                  sens_f2 + theme(legend.position = "bottom"),
                  #legends,
                  nrow = 3, ncol = 1,
                  align = "v", 
...
ggsave("/Users/dorittalia/Desktop/Stephane HTN/CVD Equity - Nature Med/Graphs/difference-EDF.png", grid, width =12, height = 20)
